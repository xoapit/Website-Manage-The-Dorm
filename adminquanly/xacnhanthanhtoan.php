...
<?php
 ...
 ?>
 <div class="container">
 <div class="boxtitle text-center" style="background:#d0493b">XÁC NHẬN THANH TOÁN</div>
 <?php
 ...
 <hr>
    ...
    <hr>
			...
			{ 				
				...
				if (isset($_POST['txtmssv'])&&isset($_POST['txtxacnhanmssv'])) {
					...
					if($txtmssv == $txtxacnhanmssv){
						$query = mysqli_query($conn,"select * from hopdong inner join sinhvien on sinhvien.mssv=hopdong.mssv where hopdong.mssv='$txtmssv'and khunha='".$_SESSION['adminquanly']."'");
						if($row= mysqli_fetch_assoc($query)){
							$queryup= mysqli_query($conn,"update hopdong set thanhtoan=N'Đã Thanh Toán' where mssv='$txtmssv'");	 echo "<hr>"."Mã Số Sinh Viên: ".$row['mssv']." Họ Và Tên: ".$row['hovaten']." Khu Nhà: ".$row['khunha']." Số Phòng: ".$row['sophong'];
     ...
     <button type="submit" class="btn btn-danger btn-block " style="padding:10px;">XÁC NHẬN</button>
    </form>
     <hr>
    </div>
